...
find which ones are the same length using words[i]
so find a word with same length then compare each letter of the word using double indexing
checker_string != '-' then word_wtv[i][j] == checker_string[j]
make sure its not a dash then check if the letter matches
if it does keep the loop going 
if it doesnt then break the loop (with j) and keep indexing through the words
once you have 50 (use counter to keep track) let the user attempt to enter attempt 5
*/